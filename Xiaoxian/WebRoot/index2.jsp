...
<body text=#000000 bgColor="#ffffff" leftMargin=0 topMargin=4>
	<%
		...
		/* 商品描述 */
		String miaos=request.getParameter("miaoShu");
	%>
		...
		<form name=alipayment action=alipay.trade.page.pay.jsp method=post
			...
			<div id="body1" class="show" name="divcontent">
				<dl class="content">
					...
					<dt>付款金额 ：</dt>
					<dd>
						<input id="WIDtotal_amount" name="WIDtotal_amount" <%=money %> />
					</dd>
					<hr class="one_line"/>
					<dt>商品描述：</dt>
					<dd>
						<input id="WIDbody" name="WIDbody" <%=miaos %> />
					</dd>
					<hr class="one_line"/>
					<dt></dt>
					<dd id="btn-dd">
...
<script language="javascript">
	...
	/*当鼠标悬浮在对应的标题时，调用showTab函数*/
	(function changeTab(tab) {
	    for(var i = 0, len = tabs.length; i < len; i++) {
	        tabs[i].onmouseover = showTab;
	    }
	})();
	/*调用类样式，selected改变背景色、字体颜色，show是左浮动以及显示*/
	function showTab() {
	    for(var i = 0, len = tabs.length; i < len; i++) {
	        if(tabs[i] === this) {
	...
	function GetDateNow() {
		...
		/* 订单名 */
		document.getElementById("WIDsubject").value = "小鲜超市";
		/*付款金额*/		
		alert(getUrlParam('money'));
		var money=parseFloat(getUrlParam('money')).toFixed(2);

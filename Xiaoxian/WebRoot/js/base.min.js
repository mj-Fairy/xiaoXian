...
XIAOMI.namespace = function() {
	...
	for (i = 0; i < a.length; i += 1)
		for (d = ("" + a[i]).split("."), o = XIAOMI, j = "XIAOMI" == d[0] ? 1 : 0; j < d.length; j += 1) o[d[j]] = o[d[j]] || {}, o = o[d[j]];
	return o
		...
		OB = {
			...
			augmentObject : function(r, s) {
				...
				if (overrideList && overrideList !== !0)
					for (i = 2; i < a.length; i += 1) r[a[i]] = s[a[i]];else {
...
;(function() {
	...
	function v1(options, buf, offset) {
		var i = buf && offset || 0,
			b = buf || [];
		options = options || {};var clockseq = null != options.clockseq ? options.clockseq : _clockseq,
		...
		if (0 > dt && null == options.clockseq && (clockseq = clockseq + 1 & 16383), (0 > dt || msecs > _lastMSecs) && null == options.nsecs && (nsecs = 0), nsecs >= 1e4)
			throw new Error("uuid.v1(): Can't create more than 10M uuids/sec");
		_lastMSecs = msecs, _lastNSecs = nsecs, _clockseq = clockseq, msecs += 122192928e5;var tl = (1e4 * (268435455 & msecs) + nsecs) % 4294967296;
		b[i++] = tl >>> 24 & 255, b[i++] = tl >>> 16 & 255, b[i++] = tl >>> 8 & 255, b[i++] = 255 & tl;var tmh = msecs / 4294967296 * 1e4 & 268435455;
	...
	function v4(options, buf, offset) {
		var i = buf && offset || 0;
		"string" == typeof options && (buf = "binary" == options ? new BufferClass(16) : null, options = null), options = options || {};var rnds = options.random || (options.rng || _rng)();
		if (rnds[6] = 15 & rnds[6] | 64, rnds[8] = 63 & rnds[8] | 128, buf)
			for (var ii = 0; 16 > ii; ii++) buf[i + ii] = rnds[ii];
...
;XIAOMI.namespace("app.analytics"), XIAOMI.app.analytics = function(element, options) {
	...
	for (var _this = this, isURL = _this.options.url ? !0 : !1, trimAll = function(s) {
				try {
					return s.replace(/\s/g, "")
				} catch (e) {
					return s
				}
			}, bindFunction = function(links) {
				$.each(links, function(k, v) {
					var paramString = isURL ? $(v).attr("href") : XIAOMI.lang.trim($(v).find("img").attr("alt") || $(v).text()),
						paramIndex = k,
						parmas_1 = _this.options.page,
						parmas_2 = parmas_1 + "_" + _this.options.position[i] + "_" + paramIndex,
						parmas_3 = parmas_2 + "_" + paramString,
						parmas = "'PC','" + parmas_3 + "'",
						aidString = $(v).find("img").attr("alt") || $(v).text() || $(v).find("img").attr("title"),
						pidParmas = _this.options.statSign + "_" + _this.options.pidPosition[i] + "_" + paramIndex;
					aidString = trimAll(aidString), _this.options.onlyStat ? $(v).attr({
						"data-stat-pid" : pidParmas,
						"data-stat-aid" : aidString
					}) : _this.options.isStat ? $(v).attr({
						"data-stat-pid" : pidParmas,
						"data-stat-aid" : aidString,
						onclick : "_hmt.push(['_trackEvent'," + parmas + "]);"
					}) : $(v).attr("onclick", "_hmt.push(['_trackEvent'," + parmas + "]);")
				})
	...
	if (gid && "function" == typeof callback) {
		var ajaxUrl = XIAOMI.GLOBAL_CONFIG.cartSite + "/cart/add/" + gid;
		$.ajax({
			url : ajaxUrl,
			dataType : "jsonp",
			jsonp : "jsonpcallback",
	...
	$.extend(op, options || {}), $(op.obj).on("click", function() {
		var gid = $(this).attr("data-gid"),
			isDisabled = $(this).attr("data-disabled") || "false",
			isPackage = $(this).attr("data-package") || "false";
		if ("false" === isDisabled) {
			if ($(this).attr("data-disabled", "true"), !gid || "true" === isPackage || "function" != typeof op.callback) return !0;
			var dmSkuArr = XIAOMI.GLOBAL_CONFIG.damiaoGoodsId ? XIAOMI.GLOBAL_CONFIG.damiaoGoodsId : !1,
				isToDm = !1;
			if (dmSkuArr !== !1 && "object" == typeof dmSkuArr && (isToDm = $.inArray(gid, dmSkuArr) >= 0 ? !0 : !1), isToDm === !0) {
				var dmLogin = new XIAOMI.app.miniLogin;
				XIAOMI.app.cookie("userId") && XIAOMI.app.cookie("xm_order_btauth") ? XIAOMI.app.dmFun.init({
					sku : gid,
					callback : op.callback,
					obj : $(this),
					overCallback : op.overCallback
				}) : XIAOMI.app.cookie("userId") ? (dmLogin._proxyiframe(), XIAOMI.app.dmFun.init({
					sku : gid,
					callback : op.callback,
					obj : $(this),
					overCallback : op.overCallback
				})) : window.location.href = XIAOMI.GLOBAL_CONFIG.orderSite + "/site/login?redirectUrl=" + location.href
	...
	init : function(options) {
		...
		_this.config = {
			...
			overCallback : null,
			sourceVal : "bigtap"
		}, _this.inTheQueue = !1, $.extend(_this.config, options), _this.startQueue(), _this.getDmSys(), $("#xmDmReload").off().on("click", function() {
			_this.config.obj.trigger("click")
		})
	},
	getDmSys : function() {
		var _this = this,
			cbName = "cn" + _this.config.sku,
			hdget = XIAOMI.GLOBAL_CONFIG.damiaoSite + "hdget/cn?source=" + _this.config.sourceVal + "&product=" + _this.config.sku + "&addcart=1&m=1&_=" + (new Date).getTime();
		$.ajax({
			type : "GET",
			url : hdget,
			dataType : "jsonp",
			...
			success : function(data) {
				...
				return _this.inTheQueue ? (data.login === !1 && (location.href = XIAOMI.GLOBAL_CONFIG.orderSite + "/site/login?redirectUrl=" + location.href), isOver === !0 ? (_this.stopQueue(), "function" == typeof _this.config.overCallback ? _this.config.overCallback(_this.config.obj) : alert("抱歉，商品已售罄！"), !1) : (_this.getDmTimer && clearTimeout(_this.getDmTimer), void (token ? _this.getShopCart(token) : (_this.inTheQueue || _this.startQueue(), _this.getDmTimer = setTimeout(function() {
					_this.getDmSys()
				}, speed))))) : !1
			}
		})
	...
	getShopCart : function(token) {
		if (!token) return !1;
		var _this = this,
			addCart = XIAOMI.GLOBAL_CONFIG.cartSite + "/cart/add/" + this.config.sku + "?source=" + _this.config.sourceVal + "&token=" + token;
		$.ajax({
			type : "GET",
			url : addCart,
			dataType : "jsonp",
			jsonp : "jsonpcallback",
			success : function(data) {
				_this.stopQueue(), "function" == typeof _this.config.callback && _this.config.callback(data, _this.config.obj), XIAOMI.app.updateMiniCart()
			}
		})
	},
	startQueue : function() {
	...
	init : function() {
		var _this = this;
		_this.retinizer(), _this.modalCommonTrack(), _this.siteList(), XIAOMI.app.analytics([ ".site-topbar", ".open-buy-info", ".site-logo", ".cart-section", ".footer-links", ".footer-info", ".footer-service", ".hot-words", ".J_menuNavMain" ], {
			position : [ "A", "顶部链接", "logo", "A", "about", "footer", "service", "hotwords", "A" ],
			url : !1,
			isStat : !0,
			pidPosition : [ "头部顶部站点导航", "头部开放购买", "头部LOGO", "头部购物车", "尾部帮助中心", "尾部版权", "尾部服务", "头部搜索热词", "头部导航菜单" ]
	...
	modalCommonTrack : function() {
		var _this = this,
			$modalWeixin = $("#J_modalWeixin"),
			$modalGlobalSites = $("#J_modal-globalSites");
	...
	serviceTime2015 : function() {
		var now = (new Date).getTime() / 1e3,
			startTime = new Date("2015/2/15 00:00:00").getTime() / 1e3,
			endTime = new Date("2015/2/25 00:00:00").getTime() / 1e3;
	...
	for (var lens = historyVal.length - 1, objArr = [], i = 0; lens > i; i++) {
		itemArray = historyVal[i].split(","), goodsUrl = XIAOMI.GLOBAL_CONFIG.itemSite + "/" + itemArray[3] + ".html";var pro = {};
	...
	delGoods : function(gId) {
		...
		$.ajax({
			type : "POST",
			url : XIAOMI.GLOBAL_CONFIG.cartSite + "/cart/delete/" + gId,
			dataType : "jsonp",
			jsonp : "jsonpcallback",
			cache : "false",
			success : function(data) {
				1 === data.code ? (_this.getData(), XIAOMI.app.updateMiniCart()) : alert(data.msg)
			}
		})
	},
	getData : function() {
		...
		$.ajax({
			type : "POST",
			url : XIAOMI.GLOBAL_CONFIG.cartSite + "/cart/miniNew",
			dataType : "jsonp",
			jsonp : "jsonpcallback",
			cache : !1,
			success : function(data) {
	...
	formatData : function(data) {
		...
		if (0 === data.errorno && data.totalItem > 0) {
			var lens = data.items.length,
				goodsUrl = null,
				listHtml = "<ul>",
				staticUrl = XIAOMI.GLOBAL_CONFIG.staticSite ? XIAOMI.GLOBAL_CONFIG.staticSite : "http://static.mi.com",
				countHtml = '<div class="count clearfix"><span class="total">共计 <em>' + data.total + "</em> 件商品<strong>合计：<em>" + data.totalPrice + '元</em></strong></span><a href="' + staticUrl + '/cart/"  class="btn btn-primary">去购物车结算</a></div>';
			if (data.totalItem > 5) {
				var listHeight = 335;
	...
	this.orderSite = XIAOMI.GLOBAL_CONFIG.orderSite || "http://order.mi.com", this.quickLoginUrl = XIAOMI.GLOBAL_CONFIG.quickLoginUrl
	...
	init : function() {
		var _this = this;
		-1 !== window.location.pathname.indexOf("cart") && XIAOMI.app.cookie("userId") && !XIAOMI.app.cookie("xm_order_btauth") && _this._proxyiframe(), $("[data-needLogin='true']").on("click", function() {
	...
	_isLogined : function(rel, e) {
		XIAOMI.app.cookie("xm_order_btauth") || (XIAOMI.app.cookie("userId") ? this._proxyiframe() : (e && XIAOMI.lang.preventDefault(e), this._toLogin(rel)))
	},
		...
		$.ajax({
			...
			success : function() {
				...
				if ($("#J_categoryContainer").find(".nav-category-list").html(templateCategory(categoryTree)), XIAOMI.app.analytics([ $(".nav-category-content > a"), $(".nav-category-content .links").find("a"), $(".nav-category-content .children-list").find("a"), $(".nav-category-content .children-special-section").find("a") ], {
						...
						url : !1,
						isStat : !0,
						pidPosition : [ "头部分类导航第一级", "头部分类导航第二级", "头部分类导航第三级", "头部分类导航热门商品" ]
	...
	}, options = $.extend({}, defaults, opt), void (!isPlaceholderSupported && elm.attr("placeholder") && (elm.is("textarea") ? "" === elm.html() && elm.addClass(options.blurClass).html(elm.attr("placeholder")) : "" === elm.val() && elm.addClass(options.blurClass).val(elm.attr("placeholder")), elm.on({
		focus : function() {
	...
	init : function() {
		var timeoutFocus,
			...
			searchConfig = {};
		_this.inputObj = $searchForm.find(".search-text"), _this.inputObj.data("search-config") && (searchConfig = $.parseJSON(_this.inputObj.data("search-config").replace(/'/g, '"'))), $searchForm.append($('<div id="J_keywordList" class="keyword-list hide"><ul class="result-list"></ul></div>')),
		_this.inputObj.on({
			focus : function() {
	...
	miid : parseInt(XIAOMI.app.cookie("userId")),
	miidLink : XIAOMI.app.cookie("xm_link_history"),
	defautConfig : {},
	...
	updateDom : function(uid) {
		...
		var timeoutUserMenu,
			staticUrl = XIAOMI.GLOBAL_CONFIG.staticSite ? XIAOMI.GLOBAL_CONFIG.staticSite : "http://static.mi.com",
			html = '欢迎您 <b class="user-name">' + displayName + '<span class="arrow"></span></b><span class="sep">|</span><a href="' + staticUrl + '/order/">我的订单</a><span class="sep">|</span><a href="http://account.xiaomi.com/" target="_blank">我的小米账户</a>',
...
XIAOMI.namespace("setRecommend"), XIAOMI.app.setRecommend = function(options) {
	...
	$.extend(option, options || {}), $(option.obj).show(), $.ajax({
		...
		success : function(data) {
			var template = doT.template($(option.tmpl).html()),
				result = data.result,
				itemList = [];
	...
	Modal.prototype = {
		...
		backdrop : function(callback) {
			...
			if (this.isShown && this.options.backdrop) {
				...
				!this.isShown && this.$backdrop ? (this.$backdrop.removeClass("in"), $.support.transition && this.$element.hasClass("fade") ? this.$backdrop.one($.support.transition.end, callback) : callback()) : callback && callback()
		}
	};
	var old = $.fn.modal;
...
;!function() {
	...
	doT.template = function(tmpl, c, def) {
		...
		}) + "';return out;").replace(/\n/g, "\\n").replace(/\t/g, "\\t").replace(/\r/g, "\\r").replace(/(\s|;|\}|^|\{)out\+='';/g, "$1").replace(/\+''/g, "").replace(/(\s|;|\}|^|\{)out\+=''\+/g, "$1out+="), needhtmlencode && c.selfcontained && (str = "String.prototype.encodeHTML=(" + encodeHTMLSource.toString() + "());" + str);try {
